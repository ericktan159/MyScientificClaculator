...
namespace MyScintificCalculator
{
    class RPN
    {
        ...
        static private string GetExpression(string input)
        {
            ...
            {
                if(IsOpertator(input[i]) || char.IsDigit(input[i]))
                {
                    if (k == ' ')
                        k = input[i];
                    else
            ...
            for (int i = 0; i < input.Length; i++)
            {
                ...
                {
                    while (!IsDelimeter(input[i]) && !IsOpertator(input[i]))
                    {
                        output += input[i];
                        i++;
        ...
        static private bool IsOpertator(char c)
        {
            if ("+-/*^()PC!%".IndexOf(c) != -1)
                return true;
            return false;
        ...
        static private String doFunc(String fun, double param)
        {
            switch(fun)
            {
                ...
                case "tg": return Math.Tan(param).ToString();      //if (Math.Abs(param % (2 * Math.PI)) == (Math.PI / 2)) throw new TgException(param); else return Math.Tan(param).ToString();
                case "asin": return Math.Asin(param).ToString();    //return  (param < -1 || param > 1) throw new ArcSinCosException(param); else return Math.Asin(param).ToString();
                case "acos": return Math.Acos(param).ToString();    //if (param < -1 || param > 1) throw new ArcSinCosException(param); else return Math.Acos(param).ToString();
                case "atg": return Math.Atan(param).ToString();
                case "sqrt": return Math.Sqrt(param).ToString();    //if (param < 0) throw new SqrtException(param); else return Math.Sqrt(param).ToString();
                case "ln": return Math.Log(param).ToString();  //if (param <= 0) throw new LogException(param); else return Math.Log(param).ToString();
                case "lg": return Math.Log10(param).ToString();  //if (param <= 0) throw new LogException(param); else return Math.Log10(param).ToString();
                default: return "";
                    //
                    //
                    //
