...
namespace MyScintificCalculator
{
    class Calculator
    {
        ...
        private void cleanText(ref string text)
        {
            ...
            var pattern4 = new Regex(@"℮\d"); // error: ℮5
            if (pattern4.IsMatch(text)) throw new Exception();
            ...
            text = text.Replace("-+", "-");
            text = text.Replace("+-", "-");
            text = text.Replace("π", Math.PI.ToString());//"3.1415926535897932384626433832795");
            text = text.Replace("℮", Math.E.ToString());//℮
        }
        ...
        {
            return text.IndexOf(subText  ) != -1;//StringComparison.OrdinalIgnoreCase) != -1;
        }
        ...
        private double executeFunction(string func, string funcText)
        {
            ...
            if (func.Equals("√"))
            {
                if(funcText.Contains("[") && funcText.Contains("]"))
                {
                ...
                {
                    //System.Windows.Forms.MessageBox.Show("sqrt 2");

                    return Math.Pow(value, 1/2.0);
                }
            }
        ...
        public string calculate(string text, out Color color)
        {
            ...
            if (resultStr.Equals("invalid syntax")) color = Color.Red;
